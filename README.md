...
With Resnet101 (last ``conv4`` layer):
  - Train on VOC 2007 trainval and test on VOC 2007 test, **75.0**. 
...
**Note**:
  ...
  - For Resnet101, we fix the first block (total 4) when fine-tuning the network, and only use ``crop_and_resize`` to resize the RoIs without max-pool. The final feature maps are average-pooled for classification and regression. All batch normalization parameters are fixed.
